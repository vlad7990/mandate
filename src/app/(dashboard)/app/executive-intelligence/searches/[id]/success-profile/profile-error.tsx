...
/**
 * Terminal failure view — generation_error is set and there is no healthy
 * version to fall back to. Retry inserts a fresh placeholder version; the
 * failed row stays in version history.
 */
  ...
  const handleRetry = () => {
    startTransition(async () => {
      try {
        ...
        console.error("[success-profile] retry failed:", e);
        toast.error(msg);
      }
    });
  };
      ...
      <div className="max-w-3xl mx-auto px-8 py-10 space-y-10">
        <div className="flex items-center gap-3 font-mono-label text-mono-label uppercase tracking-widest text-outline">
          ...
          <span className="text-error">
            Success Profile — Failed V{String(version).padStart(2, "0")}
          </span>
        </div>
        ...
        <div className="bg-surface-container-low border border-error/40 p-12 flex flex-col items-center text-center space-y-6">
          ...
          <div className="space-y-2 max-w-md">
            <h1 className="font-h1 text-h1">Profile generation failed</h1>
            <p className="text-body-main text-on-surface-variant">
              The draft for <span className="text-on-surface">{roleTitle}</span> @{" "}
