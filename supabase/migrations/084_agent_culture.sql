-- The eleventh agent principal: the Culture Agent's trail vocabulary
-- — the second of the company-side grouping, and the second
-- ZERO-NEW-GRANT slice.
--
-- Agents-as-principals slice eleven (plan in docs/handoffs/
-- NEXT-agent-culture.md, D1–D8 confirmed by the founder 2026-08-21).
-- Per D2 there are NO new policies: the projects row and the feedback
-- tail are read under 074's `projects_agent_select` and
-- `feedback_agent_select` (the interpreter's grant since slice one),
-- the merge-write rides `projects_agent_update`, and skills steer the
-- prompt under the pool's skills SELECT. The recruiter's optional
-- context string is the one human-handed input — carried verbatim
-- onto culture_context, a boolean in the trail, never trail text:
--
--   * `culture_profiled` joins the vocabulary — detail carries the
--     trigger, has_recruiter_context, feedback_count and
--     replaced_existing; names and free text never ride the trail.
--   * `record_agent_event` admits it — the allowlist at twelve.
...
-- identical to 083's), plus the one new value — §5h.
...
ALTER TABLE public.activity_events
  ADD CONSTRAINT activity_events_type_known CHECK (event_type IN (
    ...
    'company_researched',
    'hm_researched',
    -- 084: the Culture Agent's act, under its own name.
    'culture_profiled'
  ));

-- ---------------------------------------------------------------------------
-- 2. The agent door admits the eleventh principal's act
-- ---------------------------------------------------------------------------
...
BEGIN
  IF p_event_type NOT IN ('feedback_interpreted', 'candidates_ranked',
                          ...
                          'hm_researched', 'culture_profiled') THEN
