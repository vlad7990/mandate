-- The seventh agent principal: the triangulator's trail vocabulary.
--
-- Agents-as-principals slice seven (plan in docs/handoffs/
-- NEXT-agent-triangulation.md, D1–D8 confirmed by the founder
-- 2026-08-21). Per D2 this slice adds ZERO table grants — the
-- narrowest slice yet: the synthesis reads candidates and projects
-- (the company and HM reports live on projects.company_context,
-- inside 074's projects_agent_select), injects skills, and writes ONE
-- key (triangulation_report) through the 021 RPC. No web search, no
-- storage.
--
--   * `candidate_triangulated` joins the vocabulary — one event per
--     LANDED report, org visibility, trigger named in detail
--     (generate / regenerate).
--   * `record_agent_event` admits it — the allowlist at seven.
--
-- The invariants' control run for this slice regresses `is_agent()`
-- itself: re-created as a direct role read that forgets the
-- status='active' gate (which today lives in current_user_role()),
-- the "simplification" drift that would let a SUSPENDED agent through
-- the trail door.
...
-- identical to 079's), plus the one new value — §5h.
...
ALTER TABLE public.activity_events
  ADD CONSTRAINT activity_events_type_known CHECK (event_type IN (
    ...
    -- 080: the triangulator's act, under its own name.
    'candidate_triangulated'
  ));

-- ---------------------------------------------------------------------------
-- 2. The agent door admits the seventh act
-- ---------------------------------------------------------------------------
