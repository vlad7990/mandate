-- 096 — the Candidate Search Agent's vocabulary (the closing slice;
-- D3/D4 of NEXT-search-agents.md, D1–D8 confirmed 2026-08-24).
--
-- The SIXTH zero-new-grant conversion: every read the pool-search
-- judgment makes is already in the pool — candidates S, candidate_scores
-- S, projects S (074), skills S (074). This migration is vocabulary
-- only: TWO types join the CHECK (rebuilt from the LIVE pg_constraint
-- list, 62 values) and the record_agent_event allowlist grows to
-- TWENTY-FIVE.
--
--   * candidate_search_answered — one event per answered pool search,
--     counts only (pool size, filtered size, matches returned, which
--     filters were applied) — never the query text, never a name.
--   * sourcing_search_executed — minted AHEAD of its channel (the
--     slice-fourteen `scheduled` precedent): the sourcing runner is
--     seam-bound to this principal in the same slice (D8), and when
--     its connector surface ships the vocabulary is already waiting.
--     Counts only: search rounds and domain COUNT — never a domain
--     list, never a person.
...
ALTER TABLE public.activity_events
  ADD CONSTRAINT activity_events_type_known CHECK (event_type IN (
    ...
    -- 096: the Candidate Search Agent's two judgments, under its
    -- own name — the pool search live, the sourcing search minted
    -- ahead of its channel.
...
BEGIN
  IF p_event_type NOT IN ('feedback_interpreted', 'candidates_ranked',
                          ...
                          'success_profile_generated', 'interview_plan_generated',
                          'executive_context_researched',
                          'candidate_search_answered', 'sourcing_search_executed') THEN
